...
Place source PDFs in [`documents/pdfs`](./documents/pdfs). The notebook reads every PDF in that folder, including nested subfolders, splits the extracted text into chunks, and indexes those chunks in Chroma.

The Chroma index is persisted locally under `storage/chroma`. On later runs, VeriRAG compares a manifest of the PDFs, embedding model, and chunking settings against the stored index. If nothing changed, it loads the existing index instead of recomputing embeddings. If PDFs or chunking settings change, it rebuilds the index automatically.
